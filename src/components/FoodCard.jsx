...
const FoodCard = ({title1, title2, image, pop, onClick}) => {
  return (
    <div className="w-48 h-56 rounded-3xl shadow-[0_6px_17px_rgba(0,0,0,0.13)] relative cursor-pointer mb-5" onClick={onClick}>
      <img className="w-28 h-32 mx-auto" src={image} alt="" />
      <div className='ml-3 text-base mb-2'>
        <span className='block font-bold'>{title1}</span>
        <span className='block border-b'>{title2}</span>
      </div>
